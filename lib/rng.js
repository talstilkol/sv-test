// lib/rng.js
// Deterministic seeded PRNG for LumenPortal — replaces non-deterministic native random calls across app.
//
...
(function (root) {
  ...
  // We deliberately avoid native random here so the project's non-deterministic
  // random-call budget stays at zero. Mixing Date.now() + performance.now()
  ...
  function uuid4() {
    ...
    // Fallback: build 16 bytes from entropy32() chunks without native random.
  ...
  function getUserId() {
    try {
      ...
      if (!root || !root.localStorage) return "anonymous";
      const storage = root.localStorage;
      let id = storage.getItem(USER_ID_KEY);
      if (!id) {
        id = uuid4();
