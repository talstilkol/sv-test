# Execution Tasks Completed Detail Archive

Updated: 2026-04-30

This archive preserves the detailed historical task log that used to live in `EXECUTION_TASKS.md`.
The active task board now keeps remaining work first and moves completed-task detail here to reduce noise.

Rules preserved from the active plan:

- No `Math.random()` in code.
- No fake data or placeholders.
- No automatic question generation.
- Completed claims below are historical records; current truth still comes from live gates and the active task board.
...
> **עדכון אמת 2026-04-30:** המספרים ההיסטוריים מעל נשמרים לצורך רצף, אבל אינם מקור release truth. מקור האמת הנוכחי הוא [FINISH_LINE_PRERELEASE_REPORT.md](FINISH_LINE_PRERELEASE_REPORT.md): `17/18` שערים עברו, ו-Finish Line 1 חסום רק על `questions:coverage-targets:strict` עד השלמת הכיסוי הידני המלא של שאלות (`558` פערי MC concepts ו-`552` פערי Fill concepts). כל שאר שערי SVCollege/QA/build/test עברו ללא generated/seeded bank.
...
### Manual Question Guard Closure — 2026-04-30

נסגרו `FWD-0.1` עד `FWD-0.4`: נוסף gate מפורש `guard:no-auto-questions` דרך `scripts/report_no_auto_question_generation.js`. השער מוודא ש-`scripts/seed_questions.js`, `scripts/audit_seeded_questions.js` ו-`tests/seeded-qa.test.js` נשארים מחוקים; שאין script ב-`package.json` שמחזיר seed/audit generation; ש-`app.js`, `content-loader.js`, `index.html` ו-`service-worker.js` לא טוענים `questions_bank_seeded.js` ולא מכילים hooks כמו `ensureSeededBank`, `generatedMCVariantsForConcept`, `makeCodeFill`, `generatedQuestionId`; ושאין native-random API בקוד הפעיל. `data/questions_bank_seeded.js` נשאר ארכיון legacy לא פעיל בלבד עד החלטה מפורשת. נבדק: `node --check scripts/report_no_auto_question_generation.js`, `npm run guard:no-auto-questions`, `npm test -- --run tests/no-auto-question-generation.test.js`, וחיפוש native-random פעיל ללא התאמות.

### Manual Question Blocker Map — 2026-04-30

נסגרו `FWD-1.1` ו-`FWD-1.4`: נוסף דוח `questions:blocker-map` דרך `scripts/report_manual_question_blocker_map.js`, עם פלט ב-[MANUAL_QUESTION_BLOCKER_MAP.md](MANUAL_QUESTION_BLOCKER_MAP.md) ו-[MANUAL_QUESTION_BLOCKER_MAP.json](MANUAL_QUESTION_BLOCKER_MAP.json). הדוח לא מייצר שאלות ולא משנה בנק שאלות; הוא רק ממפה את שני release blockers החיים מתוך `svcollege:readiness:release`.

ממצאים חיים: שני החסמים הם `עיצוב רספונסיבי ו-CSS מתקדם` (`lesson_25`) ו-`AI למפתחים — Cursor, Windsurf, Bolt, תיעוד וטסטים עם AI` (`ai_development`). יש `9` מושגים לחיבור ידני עכשיו: `lesson_25::Tailwind CSS`, `lesson_25::utility classes`, `lesson_25::responsive design`, `lesson_25::grid`, `lesson_25::flex`, `ai_development::Cursor`, `ai_development::Windsurf`, `ai_development::Prompt Engineering`, `ai_development::AI Code Review`. כולם עם `0` MC ו-`0` Fill בבנק הידני, כולם דורשים Fill כי יש להם code example. מינימום לפתיחת release readiness: `18` שאלות ידניות. יעד הכיסוי המחמיר: `27` MC ידניות ו-`18` Fill ידניות. נבדק: `node --check scripts/report_manual_question_blocker_map.js`, `npm run questions:blocker-map:write`.

### Manual Blocker Authoring Closure — 2026-04-30

נסגרו `FWD-1.2` עד `FWD-1.11`: נכתבו ידנית `27` שאלות MC ו-`18` שאלות Fill ל-`9` מושגי החסם (`lesson_25` ו-`ai_development`) בתוך `data/questions_bank.js`; נוסף feedback ידני לכל distractor ב-`data/option_feedback.js`; נוספו בדיקות `tests/manual-blocker-question-authoring.test.js` ו-`tests/manual-question-blocker-map.test.js`; ודוח `MANUAL_QUESTION_BLOCKER_MAP` עודכן ל-`releaseQuestionDeficit: 0`, `strictMcDeficit: 0`, `strictFillDeficit: 0`. `svcollege:readiness:release`, `svcollege:tab-matrix:strict`, `svcollege:critical-flows:strict`, `svcollege:command-center:strict`, `svcollege:student-export:strict`, `exam:mock-variants:strict` ו-`svcollege:console-gate:strict` עברו בלי generated bank.

נסגר גם `FWD-2.1`: נוסף דוח מלא [MANUAL_QUESTION_AUTHORING_PLAN.md](MANUAL_QUESTION_AUTHORING_PLAN.md) / [MANUAL_QUESTION_AUTHORING_PLAN.json](MANUAL_QUESTION_AUTHORING_PLAN.json) דרך `scripts/report_manual_question_authoring_plan.js`. הדוח מחלק את פערי הכיסוי הידני שנותרו ל-`28` באטצ'ים, עם owner/reviewer אמיתיים עדיין `unknown/unavailable` עד הקצאה. מצב חי אחרי הבאטצ' הראשון: `questions:coverage-targets:strict` עדיין אדום בכוונה עם `558` concepts חסרי MC מלא ו-`552` concepts חסרי Fill מלא. כמות השאלות שעדיין נדרשת ליעד strict מלא: `1,572` MC ידניות ו-`1,007` Fill ידניות.

נסגר `FWD-7.1`: נוסף gate מאוחד `finish-line:pre-release` דרך `scripts/report_finish_line_prerelease.js`, עם דוח [FINISH_LINE_PRERELEASE_REPORT.md](FINISH_LINE_PRERELEASE_REPORT.md) / [FINISH_LINE_PRERELEASE_REPORT.json](FINISH_LINE_PRERELEASE_REPORT.json). הריצה האחרונה הפעילה את כל שערי Finish Line ברצף והמשיכה גם אחרי כשל כדי להציג תמונת אמת מלאה: `17/18` עברו; החסם היחיד הוא `questions:coverage-targets:strict`. הסקריפט לא יוצר שאלות, לא טוען seeded bank, ומבצע סריקת native-random פנימית לפני השערים.

נסגרו `FWD-3.1` ו-`BUG-AUDIT-002`: נוספו ידנית `9` Trace items ל-Design Systems ב-`data/svcollege_traces_design_systems.js` עבור `shadcn/UI`, `Radix primitives`, `design tokens`, `cn helper`, `cva`, `asChild slot`, `theme tokens`, `component registry`, `design system testing`. [QUESTION_ACTIVITY_COVERAGE_REPORT.md](QUESTION_ACTIVITY_COVERAGE_REPORT.md) ירד מ-`231` ל-`222` פערים, ו-[QUESTION_ACTIVITY_AUTHORING_PLAN.md](QUESTION_ACTIVITY_AUTHORING_PLAN.md) מציג `svcollegePriorityGaps: 0`. `questions:activity-authoring-plan:strict` עדיין נכשל בכוונה על `222` פערים לא-עדיפותיים עד שיש activity authoring ידני.

### Lesson Menu Density Addendum — 2026-04-30

בוצע צמצום תפריטי השיעור לפי דרישת המשתמש: במקום תפריטי `details` מקוננים שתופסים גובה, מסך שיעור מציג עכשיו עץ שורות קומפקטי בלבד: שורת מסלול, שורת מושגים אופקית, ושורת טאבי שיעור. כל שורה מציגה תיאור מימין ורשימה נגללת משמאל. שורות תחתונות מוצגות רק כאשר האב שלהן פתוח. `חלקי המושג` הועברו לתפריט התצוגה השמאלי כדי לא להוסיף שורת chrome נוספת מעל התוכן. עודכן cache shell ל-`concept-sprint-v69` / `lumen-v2.4.130`. נבדק מקומית: `node --check app.js`, בדיקות תפריטים/PWA, `npm test -- --run`, `npm run svcollege:pwa-offline:strict`, `npm run validate:strict`, `npm run qa:questions:strict`, `npm run svcollege:readiness:release`, `npm run svcollege:tab-matrix:strict`, `npm run svcollege:command-center:strict`, `npm run svcollege:student-export:strict`, `npm run build`, ו-`git diff --check`. עדיין נדרש visual smoke בדפדפן לפני סימון browser QA מלא.

### Kimi 2.6 f5798 Reconciliation — 2026-04-30
...
- [V] FWD-0.1 — להשאיר את `scripts/seed_questions.js`, `scripts/audit_seeded_questions.js`, `tests/seeded-qa.test.js` מחוקים, ולוודא ש-`package.json` לא מחזיר `audit:seeded` או seed script.
- [V] FWD-0.2 — להוסיף/להקשיח release guard שסורק שאין `ensureSeededBank`, `generatedMCVariantsForConcept`, `makeCodeFill`, `generatedQuestionId`, `questions_bank_seeded.js` ב-runtime או ב-service worker.
- [V] FWD-0.3 — להריץ בכל batch: `rg -n "Math\\.random" app.js src scripts tests data --glob '!output/**'`; חייב להיות ללא התאמות.
- [V] FWD-0.4 — לשמור את `data/questions_bank_seeded.js` כארכיון לא פעיל בלבד או למחוק אותו רק אחרי החלטה מפורשת; בשום מצב לא להטעין אותו בדפדפן או בדוחות readiness.

#### P0 — שני מודולי SVCollege שחוסמים Trainer/Mock Exam

- [V] FWD-1.1 — למפות ידנית את כל מושגי `עיצוב רספונסיבי ו-CSS מתקדם` שחסרים MC/Fill ב-`QUESTION_COVERAGE_TARGETS.md`.
- [V] FWD-1.2 — לכתוב שאלות MC ידניות למודול `עיצוב רספונסיבי ו-CSS מתקדם`: לכל שאלה מקור מושג, תשובה נכונה, 3 distractors סבירים, `explanation`, `optionFeedback`, `level`, `conceptKey`.
- [V] FWD-1.3 — לכתוב שאלות Fill ידניות למושגי `lesson_25` עם code example: blank אחד ברור, answer יחיד, hint שאינו מגלה, explanation.
- [V] FWD-1.4 — למפות ידנית את כל מושגי `AI למפתחים — Cursor, Windsurf, Bolt, תיעוד וטסטים עם AI` שחסרים MC/Fill.
- [V] FWD-1.5 — לכתוב שאלות MC ידניות למודול `AI למפתחים` לפי אותה מדיניות.
- [V] FWD-1.6 — לכתוב שאלות Fill ידניות למודול `ai_development` רק איפה שיש קוד/פקודה/תבנית אמיתית שמצדיקה Fill.
- [V] FWD-1.7 — לעדכן `data/questions_bank.js` בלבד; לא ליצור קובץ generated חדש.
- [V] FWD-1.8 — לעדכן `data/option_feedback.js` לכל MC חדש.
- [V] FWD-1.9 — להוסיף/לעדכן tests ממוקדים למודולים החסרים כך שיכסו count, conceptKey, levels, option feedback ו-Mock Exam routing.
- [V] FWD-1.10 — DoD: `svcollege:readiness:release`, `svcollege:tab-matrix:strict`, `svcollege:critical-flows:strict`, `svcollege:command-center:strict`, `svcollege:student-export:strict`, `exam:mock-variants:strict`, `svcollege:console-gate:strict` עוברים בלי generated bank.
- [V] FWD-1.11 — להריץ `questions:blocker-map:strict` לפני ואחרי כל batch שאלות ידני; ה-deficits חייבים רק לרדת, ולא לספור `lesson_design_systems`/`lesson_ai_engineering` כתחליף ל-`lesson_25`/`ai_development`.

#### P0 — כיסוי שאלות ידני מלא לכל הפורטל

- [V] FWD-2.1 — להפוך את `QUESTION_COVERAGE_TARGETS.md/json` לתוכנית batch: `558` פערי MC ו-`552` פערי Fill, לפי מודול, מושג, עדיפות מבחן, ותאריך סקירה.
- [V] FWD-2.2 — להתחיל ממודולי SVCollege שנמצאים במבחן ולא ממוזיאון/חנות/קהילה.
...
- [V] FWD-3.1 — לסגור קודם את `9` פערי SVCollege-priority מתוך `QUESTION_ACTIVITY_AUTHORING_PLAN.md`.
...
- [V] FWD-5.5 — לצמצם את chrome העליון של שיעור לעץ שורות קומפקטי: מסלול, מושגים, טאבים; תיאור מימין ורשימה נגללת משמאל.
- [V] FWD-5.6 — להעביר את `חלקי המושג` לתפריט התצוגה השמאלי כדי שלא יוסיף שורה עליונה נוספת מעל התוכן.
...
- [V] FWD-7.1 — להוסיף pre-release command שמריץ יחד: no-auto-question-generation scan, no-native-random, strict QA, SVCollege gates, tests, build (`finish-line:pre-release`, [FINISH_LINE_PRERELEASE_REPORT.md](FINISH_LINE_PRERELEASE_REPORT.md)).
- [V] FWD-7.2 — להקשיח `questions:activity-authoring-plan:strict` כך שייכשל כאשר `ready:false` ויש פערי Trace/Build/Bug ידניים פתוחים.
...
- [V] FWD-9.1 — `npm run validate:strict`
- [V] FWD-9.2 — `npm run qa:questions:strict`
- [V] FWD-9.3 — `npm run quality:questions:strict`
...
- [V] FWD-9.5 — `npm run questions:reuse-audit:strict`
- [V] FWD-9.6 — `npm run svcollege:readiness:release`
- [V] FWD-9.7 — `npm run svcollege:tab-matrix:strict`
- [V] FWD-9.8 — `npm run svcollege:critical-flows:strict`
- [V] FWD-9.9 — `npm run svcollege:command-center:strict`
- [V] FWD-9.10 — `npm run svcollege:student-export:strict`
- [V] FWD-9.11 — `npm run exam:mock-variants:strict`
- [V] FWD-9.12 — `npm run svcollege:console-gate:strict`
- [V] FWD-9.13 — `npm run svcollege:pwa-offline:strict`
- [V] FWD-9.14 — `npm test -- --run`
- [V] FWD-9.15 — `npm run build`
- [V] FWD-9.16 — `rg -n "Math\\.random" app.js src scripts tests data --glob '!output/**'` ללא התאמות.
- [V] FWD-9.17 — `npm run guard:no-auto-questions`
- [V] FWD-9.18 — `npm run questions:blocker-map:strict`
