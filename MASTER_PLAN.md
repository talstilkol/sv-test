...
|---|---|---:|---|
| SYS-AUDIT-001 | [ ] | P0 | לסגור את `questions:coverage-targets:strict`: `495` פערי MC ו-`490` פערי Fill, ללא יצירה אוטומטית וללא שאלות לא-בדוקות. |
| SYS-AUDIT-002 | [ ] | P0 | להשלים את `MANUAL_QUESTION_AUTHORING_PLAN.md`: `1,411` MC ו-`906` Fill ידניות, עם owner/reviewer אמיתיים לכל batch. |
...
| SYS-AUDIT-004 | [ ] | P1 | לטפל ב-`BRUTAL_MASTER_PLAN_AUDIT`: מתוך `238` פריטים יש `216` NOT DONE ו-`7` PARTIAL; אין להציג phase כגמור עד שיש ראיית gate. |
...
| BUG-AUDIT-021 | [ ] | P0 | לכתוב/לסקור ידנית MC/Fill במקום להסתמך על legacy generated bank; אחרי תת-batch שני של Lesson 13 `questions:coverage-targets` מציג אמת: `495` פערי MC ו-`490` פערי Fill ידניים. |
...
| BUG-AUDIT-023 | [ ] | P0 | לתקן overclaim בסטטוס התוכנית: `BRUTAL_MASTER_PLAN_AUDIT.md` מצא עכשיו `238` פריטים, מתוכם `15` DONE, `7` PARTIAL, `216` NOT DONE ו-`0` FAKED. אין להציג Phase כ-100% עד שכל פריט PARTIAL מקבל ראיה אמיתית או מסומן מחדש. |
...
| BUG-AUDIT-028 | [V] | P0 | נוספה ומתעדכנת `MANUAL_QUESTION_AUTHORING_PLAN.md/json`; אחרי תת-batch שני של Lesson 13 נותרו `25` באטצ'ים, `495` פערי MC ו-`490` פערי Fill; owner/reviewer נשארים `unknown/unavailable` עד הקצאה אמיתית. |
...
| BUG-AUDIT-039 | [ ] | P0 | להשלים את הכיסוי הידני המלא שנותר לפי `MANUAL_QUESTION_AUTHORING_PLAN.md`: `1,411` MC ידניות ו-`906` Fill ידניות עדיין נדרשות כדי ש-`questions:coverage-targets:strict` יהיה ירוק לכל 568 המושגים. `FINISH_LINE_PRERELEASE_REPORT.md` מאשר שזה החסם היחיד מתוך `18` שערי pre-release. |
