...
> **עדכון System Bug Audit 2026-05-01:** [SYSTEM_BUG_AUDIT_REPORT.md](SYSTEM_BUG_AUDIT_REPORT.md) רוענן לפי gates חיים. מצב אמת: `finish-line:pre-release` הוא `17/18`, החסם היחיד הוא `questions:coverage-targets:strict`, `QUESTION_ACTIVITY_AUTHORING_PLAN` מציג `222` פערי activity לא-עדיפותיים ו-`svcollegePriorityGaps: 0`, ו-`BRUTAL_MASTER_PLAN_AUDIT` מציג `249` פריטים: `DONE 22`, `FAKED 0`, `PARTIAL 7`, `NOT DONE 220`.

> **עדכון Manual Questions 2026-05-01:** חל איסור מוחלט על יצירת שאלות אוטומטיות. `scripts/seed_questions.js`, `scripts/audit_seeded_questions.js` ו-`tests/seeded-qa.test.js` נמחקו; `content-loader.js` לא טוען יותר `questions_bank_seeded.js`; כלי validation/coverage כבר לא קוראים את archive; והדוחות סופרים readiness רק מתוך בנק ידני. אחרי QMAN-001I, מצב אמת: `questions:coverage-targets` מדווח `ready:false`, עם `471` פערי MC ו-`466` פערי Fill ידניים; `MANUAL_QUESTION_AUTHORING_PLAN.md` דורש עדיין `1,347` MC ו-`865` Fill ידניות. הבנק הידני הפעיל עומד על `357` MC ו-`271` Fill.

> **עדכון Quality Remediation 2026-05-01:** תור הערות האיכות הידני סגור גם אחרי QMAN-001I: `quality:questions:strict` מציג `628` שאלות נקיות, `0` blockers, `0` warnings ו-`0` notes; `quality:remediation:strict` מציג `total: 0`. זה לא סוגר את Finish Line 1, כי `questions:coverage-targets:strict` עדיין אדום.
...
## 0A. Current Verified System Bug Backlog — 2026-05-01
...
|---|---|---:|---|
| SYS-AUDIT-001 | [ ] | P0 | לסגור את `questions:coverage-targets:strict`: `471` פערי MC ו-`466` פערי Fill, ללא יצירה אוטומטית וללא שאלות לא-בדוקות. |
| SYS-AUDIT-002 | [ ] | P0 | להשלים את `MANUAL_QUESTION_AUTHORING_PLAN.md`: `1,347` MC ו-`865` Fill ידניות, עם owner/reviewer אמיתיים לכל batch. |
...
| SYS-AUDIT-004 | [ ] | P1 | לטפל ב-`BRUTAL_MASTER_PLAN_AUDIT`: מתוך `249` פריטים יש `220` NOT DONE ו-`7` PARTIAL; אין להציג phase כגמור עד שיש ראיית gate. |
...
| SYS-AUDIT-012 | [V] | P1 | נסגרו 17 הערות איכות ב-`quality:remediation:strict`: 12 ניסוחים גנריים, 4 Fill ambiguity ו-1 option-length balance; `quality:questions:strict` ו-`quality:remediation:strict` מדווחים עכשיו 0 הערות. |
...
| BUG-AUDIT-021 | [ ] | P0 | לכתוב/לסקור ידנית MC/Fill במקום להסתמך על legacy generated bank; אחרי תת-batch שישי של Lesson 13 `questions:coverage-targets` מציג אמת: `471` פערי MC ו-`466` פערי Fill ידניים. |
...
| BUG-AUDIT-023 | [ ] | P0 | לתקן overclaim בסטטוס התוכנית: `BRUTAL_MASTER_PLAN_AUDIT.md` מצא עכשיו `249` פריטים, מתוכם `22` DONE, `7` PARTIAL, `220` NOT DONE ו-`0` FAKED. אין להציג Phase כ-100% עד שכל פריט PARTIAL מקבל ראיה אמיתית או מסומן מחדש. |
...
| BUG-AUDIT-028 | [V] | P0 | נוספה ומתעדכנת `MANUAL_QUESTION_AUTHORING_PLAN.md/json`; אחרי תת-batch שישי של Lesson 13 נותרו `24` באטצ'ים, `471` פערי MC ו-`466` פערי Fill; owner/reviewer נשארים `unknown/unavailable` עד הקצאה אמיתית. |
...
| BUG-AUDIT-039 | [ ] | P0 | להשלים את הכיסוי הידני המלא שנותר לפי `MANUAL_QUESTION_AUTHORING_PLAN.md`: `1,347` MC ידניות ו-`865` Fill ידניות עדיין נדרשות כדי ש-`questions:coverage-targets:strict` יהיה ירוק לכל 568 המושגים. `FINISH_LINE_PRERELEASE_REPORT.md` מאשר שזה החסם היחיד מתוך `18` שערי pre-release. |
