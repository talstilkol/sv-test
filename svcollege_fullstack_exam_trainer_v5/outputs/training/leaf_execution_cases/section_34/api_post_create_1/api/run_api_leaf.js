...
async function run() {
  ...
  await safe("POST create valid", async () => { const res = await supertest(app).post("/api/items").send({ id: 3, name: "New Item", subject: "Math", text: "hello", value: 42, salary: 42, grade: 90 }); return [200, 201].includes(res.status); }); await safe("POST create invalid", async () => { const res = await supertest(app).post("/api/items").send({}); return [400, 409, 422].includes(res.status); });
