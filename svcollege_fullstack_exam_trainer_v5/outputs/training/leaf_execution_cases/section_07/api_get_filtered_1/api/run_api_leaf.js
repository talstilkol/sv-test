...
async function run() {
  ...
  await safe("POST filter", async () => { const res = await supertest(app).post("/api/items/filter").send({ value: 50, maxSalary: 50, max_salary: 50, salary: 50, threshold: 50 }); return res.status < 500 && Array.isArray(res.body); });
