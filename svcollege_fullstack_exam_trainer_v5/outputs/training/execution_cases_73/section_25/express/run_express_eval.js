...
async function run() {
  ...
  await safe("GET all", async () => { const res = await supertest(app).get("/api/items"); return res.status === 200 && Array.isArray(res.body); });
  
  
  
  await safe("PUT missing", async () => { const res = await supertest(app).put("/api/items/missing").send({ name: "Updated" }); return res.status === 404; });
  await safe("DELETE missing", async () => { const res = await supertest(app).delete("/api/items/missing"); return res.status === 404; });
