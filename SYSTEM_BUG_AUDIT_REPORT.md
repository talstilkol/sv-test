...
| `npm run master-plan:brutal-audit:write` | 226 items audited: DONE 8, FAKED 0, PARTIAL 6, NOT DONE 212 |
| `npm run questions:coverage-targets:strict` | Failed as expected: `mcGapCount: 520`, `fillGapCount: 515`, `ready:false` |
...
| `npm run metrics:dashboard:strict` | Passed, ready true, questionQualityIndex 100, totalQuestions 409; generated markdown still has older date metadata |
...
The main current blocker is honest learning coverage under the manual-only policy. `questions:coverage-targets:strict` still fails with `520` concepts missing enough MC coverage and `515` concepts missing enough Fill coverage. `MANUAL_QUESTION_AUTHORING_PLAN.md` says `1,482` MC questions and `949` Fill questions still need to be manually authored and reviewed to reach strict density across all 568 concepts.

The second major issue is planning debt. `BRUTAL_MASTER_PLAN_AUDIT.md` now audits the active plan at 226 items: only 8 are verified DONE, 6 are PARTIAL, and 212 are NOT DONE. The good news is that the current strict audit reports `FAKED: 0`; the remaining problem is volume and verification, not known pretending in the current active slice.
...
|---|---|---|---|---|---|
| SYS-AUDIT-001 | P0 | Manual question coverage | Finish Line 1 is not release-ready because strict manual MC/Fill density is still incomplete. | `questions:coverage-targets:strict`: `mcGapCount: 520`, `fillGapCount: 515`; `FINISH_LINE_PRERELEASE_REPORT.md`: 17/18 passed, 1 failed. | Continue `MANUAL_QUESTION_AUTHORING_PLAN.md` batches. A batch is DONE only after manual authoring, manual review, full option feedback, QA, source lesson verification, and strict gates. |
| SYS-AUDIT-002 | P0 | Manual authoring capacity | The remaining strict deficit is large: `1,482` MC and `949` Fill questions. | `MANUAL_QUESTION_AUTHORING_PLAN.md`. | Assign real owner/reviewer per batch. Keep owner/reviewer as `unknown/unavailable` until a real person is assigned. |
...
| SYS-AUDIT-004 | P1 | Plan truth | The active master plan still has 212 NOT DONE and 6 PARTIAL items. | `BRUTAL_MASTER_PLAN_AUDIT.md`: 226 items, DONE 8, FAKED 0, PARTIAL 6, NOT DONE 212. | Keep `master-plan:brutal-audit:write` as a regular gate and do not use phase totals as release truth. |
...
4. Fix report metadata/scope drift so every green gate can be traced to the right current source.
5. Inventory the active `innerHTML`/`document.write`/`localStorage` trust boundaries before adding more UI surface.
6. Run a real 10-student pilot before claiming learner outcome improvements.
7. Only after Finish Line 1 is green, resume modularization, Sync live-backend proof, museum/store/community, SEO, telemetry, and payload optimization.
